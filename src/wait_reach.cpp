...
int main() {
            ...
            if (type == "<INIT" and type_element.substr(0, 3) == "hai") {
                ...
                if (type_element == "who") {
                    ...
                    if (step == 2) {
                        ...
                        auto wait = list_wait(hand_kind, pattern, mianzi);
                        for (auto &&x : wait) {
                            count_wait[x]++;
                        }
                        reach_total++;
    ...
    for (int i = 0; i < 34; i++) {
        std::cout << pai_text[kind(4 * i + 1)] << '\t' << count_wait[i] << '\t'
                  << double(count_wait[i]) / reach_total << std::endl;
    }
