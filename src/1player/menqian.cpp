...
constexpr int kind = 34, tile = 4 * kind, turn = 18, iter = 10000;
...
int count_mianzi(std::vector<int> hand, int idx = 0) {
    int count = count_chows(hand);
    for (int h = 0; h < kind; h++) {
        if (hand[h] < 3 or h < idx) continue;
        hand[h] -= 3;
        count = std::max(count, 1 + count_mianzi(hand, h + 1));
        hand[h] += 3;
    }
    return count;
}

int count_mianzi_head(std::vector<int> hand) {
    int count = count_mianzi(hand), count_head = -1;
    for (int h = 0; h < kind; h++) {
        if (hand[h] < 2) continue;
        hand[h] -= 2;
        count_head = std::max(count_head, count_mianzi(hand));
        hand[h] += 2;
        if (count_head == count) break;
    }
    if (count > count_head) count--;
    return count;
}
...
void simulate(std::vector<int> &magic, std::vector<int> &real) {
    ...
    for (int i = 0; i < turn; i++) {
        ...
        seen.at(k / 4)++;
        hand.at(k / 4)++;
        if (not win_seen and count_mianzi_head(seen) >= 4) {
            win_seen = true;
        }
        if (not win_real and count_mianzi_head(hand) >= 4) {
            win_real = true;
        }
        if (win_real) {
            hand.at(k / 4)--;
        } else {
            int chows_before = count_mianzi_head(hand);
            auto v = value;
            for (int j = 0; j < kind; j++) {
                ...
                for (int l = 0; l < kind; l++) {
                    ...
                    for (int m = l; m < kind; m++) {
                        ...
                        hand.at(m)++;
                        if (count_mianzi_head(hand) > chows_before) {
                            wait += n;
                        }
                        hand.at(m)--;
                    }
