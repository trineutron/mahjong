...
bool contain_2triples(const std::vector<int> &hand) {
    int count_triples = 0;
...
void simulate(std::vector<int> &magic, std::vector<int> &real) {
    ...
    for (int i = 0; i < turn; i++) {
        bool peng_seen = false, peng_real = false;
        for (int p = 0; p < player; p++) {
            ...
            draw.at(k) = true;
            if (not peng_seen and not win_seen) {
                seen.at(k / 4)++;
                if (contain_2triples(seen)) {
                    win_seen = true;
                }
                if (p != 3) {
                    if (seen.at(k / 4) == 3) {
            ...
            if (not peng_real and not win_real) {
                hand.at(k / 4)++;
                if (contain_2triples(hand)) {
                    win_real = true;
                }
                int discard = k / 4;
                if (p == 3 or seen.at(k / 4) == 3) {
